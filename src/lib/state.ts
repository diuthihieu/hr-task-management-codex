...
export const STORAGE_KEY = "orbit-base:mvp:v4";
export const LEGACY_STORAGE_KEYS = ["orbit-base:mvp:v3", "orbit-base:mvp:v2", "orbit-base:mvp:v1"] as const;
...
export function normalizeAppState(value: unknown, migratingLegacy = false): AppState {
  ...
  const table = base?.tables.find((item) => item.id === next.activeTableId) ?? base?.tables[0];
...
function normalizeView(view: SavedView): SavedView {
  return {
    ...
    rowHeight: view.rowHeight ?? "compact",
